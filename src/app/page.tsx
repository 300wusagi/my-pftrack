...
export default function PortfolioApp() {
  ...
  const [tradeForm, setTradeForm] = useState<any>({
    id: null, trackType: 'auto', symbol: '', name: '', quantity: '', price: '',
    currency: 'USD', customPrice: '', customCurrency: 'USD', tags: {}, cashOp: 'add', amount: '',
  });
  ...
  const formatMoney = (val: number | null | undefined) => {
    ...
    return val.toLocaleString(undefined, {
      ...
      maximumFractionDigits: settings.precision ?? 2,
    });
  };
  ...
  useEffect(() => {
    ...
    if (savedSet) {
      setSettings({
        ...
        precision: savedSet.precision !== undefined ? savedSet.precision : 2,
      });
    }
    ...
    holdings.forEach(async (h: any) => {
      ...
      try {
        ...
        if (res.ok) {
          ...
          setLiveData((prev) => ({
            ...prev,
            [h.symbol]: { price: data.price, currency: data.currency, name: data.name },
          }));
        } else {
  ...
  useEffect(() => {
    ...
    setSnapshots((prev) => {
      ...
      if (exists)
        return prev.map((s) => s.date === today ? { ...s, value: totalValue } : s);
      ...
      hld.map((h) => {
        if (h.id === tradeForm.id) {
          return {
            ...
            customPrice: tradeForm.trackType === 'custom' ? parseFloat(tradeForm.customPrice) || price : null,
            customCurrency: tradeForm.trackType === 'custom' ? tradeForm.customCurrency : null,
            tags: tradeForm.tags,
          };
        }
        return h;
  ...
  const handleCashSubmit = (e: React.FormEvent) => {
    ...
    setCash((prev) => {
      ...
      if (op === 'set') {
        if (existing) return prev.map((c) => c.currency === cur ? { ...c, amount: amt } : c);
        return [...prev, { id: Date.now().toString(), currency: cur, amount: amt }];
      }
      if (existing) {
        return prev.map((c) => c.currency === cur ? { ...c, amount: op === 'add' ? c.amount + amt : Math.max(0, c.amount - amt) } : c);
      }
      return op === 'add' ? [...prev, { id: Date.now().toString(), currency: cur, amount: amt }] : prev;
    });
    setModalType(null);
  };
  ...
  const handleSellSubmit = (e: React.FormEvent) => {
    ...
    const price = parseFloat(tradeForm.price);
    const cur = tradeForm.currency;
    if (qty > sellTarget.quantity) return alert(`最多可卖出 ${sellTarget.quantity}`);

    const proceeds = qty * price;
    setCash((prev) => {
      const existing = prev.find((c) => c.currency === cur);
      if (existing) return prev.map((c) => c.currency === cur ? { ...c, amount: c.amount + proceeds } : c);
      return [...prev, { id: Date.now().toString(), currency: cur, amount: proceeds }];
    });

    if (qty === sellTarget.quantity) {
      setHoldings((prev) => prev.filter((h) => h.id !== sellTarget.id));
    } else {
      setHoldings((prev) => prev.map((h) => h.id === sellTarget.id ? { ...h, quantity: h.quantity - qty } : h));
    }
    setModalType(null);
  };
  ...
  const handleBuySubmit = (e: React.FormEvent) => {
    ...
    setCash((prevCash) =>
      hasCashAcc
        ? prevCash.map((c) => c.currency === tradeForm.currency ? { ...c, amount: c.amount - cost } : c)
        : [...prevCash, { id: Date.now().toString(), currency: tradeForm.currency, amount: -cost }]
    );
    ...
    if (existing) {
      ...
      setHoldings((hld) => hld.map((h) => h.id === existing.id ? { ...h, quantity: newQty, costPrice: newCost } : h));
    } else {
      setHoldings((hld) => [
        ...hld,
        {
          id: Date.now().toString(), trackType: tradeForm.trackType, symbol: sym, name: tradeForm.name || sym,
          quantity: qty, costPrice: parseFloat(tradeForm.price), currency: tradeForm.currency,
          customPrice: parseFloat(tradeForm.customPrice) || parseFloat(tradeForm.price),
          customCurrency: tradeForm.customCurrency, tags: tradeForm.tags,
        },
      ]);
    }
    setModalType(null);
  ...
  const getAllocData = (dim: string) => {
    ...
    if (dim === 'holding') {
      holdings.forEach((h: any) => {
        ...
        sums[h.symbol] = (sums[h.symbol] || 0) + convert((h.trackType === 'auto' ? live.price : h.customPrice) * h.quantity, h.trackType === 'auto' ? live.currency : h.customCurrency);
      });
      cash.forEach((c: any) => { sums[`💵 ${c.currency}`] = (sums[`💵 ${c.currency}`] || 0) + convert(c.amount, c.currency); });
    } else if (dim === 'currency') {
      holdings.forEach((h: any) => {
        const live = liveData[h.symbol];
        if (h.trackType === 'auto' && (!live || live.error)) return;
        const cur = h.trackType === 'auto' ? live.currency : h.customCurrency;
        sums[cur] = (sums[cur] || 0) + convert((h.trackType === 'auto' ? live.price : h.customPrice) * h.quantity, cur);
      });
      cash.forEach((c: any) => { sums[c.currency] = (sums[c.currency] || 0) + convert(c.amount, c.currency); });
      ...
      holdings.forEach((h: any) => {
        ...
        sums[tagVal] = (sums[tagVal] || 0) + convert((h.trackType === 'auto' ? live.price : h.customPrice) * h.quantity, h.trackType === 'auto' ? live.currency : h.customCurrency);
      });
      const totalCash = cash.reduce((sum: number, c: any) => sum + convert(c.amount, c.currency), 0);
      if (totalCash > 0) sums['💵 现金'] = totalCash;
    }
    return Object.keys(sums).map((k) => ({ name: k, value: sums[k] })).filter((d) => d.value > 0).sort((a, b) => b.value - a.value);
  };
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200 font-sans">
        <div className="flex justify-between items-center mb-6 text-gray-800">
          ...
          <div className="space-x-4">
            <button onClick={prevMonth} className="hover:text-blue-600 transition-colors font-bold text-sm text-gray-500">◀ 上月</button>
            <button onClick={nextMonth} className="hover:text-blue-600 transition-colors font-bold text-sm text-gray-500">下月 ▶</button>
          </div>
        </div>
        <div className="grid grid-cols-7 gap-px bg-gray-200 border border-gray-200 rounded-lg overflow-hidden">
          {['日', '一', '二', '三', '四', '五', '六'].map((d) => (
            <div key={d} className="bg-gray-50 text-center py-2 text-xs font-bold text-gray-500">{d}</div>
          ))}
          {blanks.map((_, i) => (
          ...
          {days.map((day) => {
            ...
            return (
              <div key={day} className={`bg-white min-h-[100px] p-2 flex flex-col justify-between transition-colors ${snap ? 'hover:bg-gray-50' : ''} border-t border-r border-gray-100`}>
                <div className="text-gray-500 text-xs font-bold">{day}</div>
                {snap && (
                  <div className="text-center pb-2">
                    <div className={`text-xs font-bold ${isPositive ? 'text-red-500' : isNegative ? 'text-green-600' : 'text-gray-400'}`}>
                      {isPositive ? '▲' : isNegative ? '▼' : ''} {formatMoney(Math.abs(snap.diffAmount))}
                    </div>
                    <div className={`text-[10px] mt-0.5 ${isPositive ? 'text-red-400' : isNegative ? 'text-green-500' : 'text-gray-400'}`}>
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center font-bold text-gray-400">Loading...</div>;return (
    <div className="min-h-screen bg-gray-50 text-gray-800 font-sans pb-20">
      <div className="bg-white border-b sticky top-0 z-30 shadow-sm px-6 py-4 flex justify-between items-center">
        <div>
          <h1 className="text-xl font-black text-blue-600">PF<span className="text-gray-800">TRACK</span></h1>
        </div>
        <div className="text-right">
          <div className="text-2xl font-bold">{formatMoney(totalValue)}</div>
          <div className="text-xs text-gray-500 font-bold">总资产 ({settings.baseCurrency})</div>
        </div>
      </div>
        ...
        {activeTab === 'holdings' && (
          <div className="space-y-6">
            <div className="flex justify-between items-center flex-wrap gap-4">
              ...
              <div className="flex gap-2 relative flex-wrap">
                ...
                >
                  ...
                  {tagGroups.map((g) => <option key={g.name} value={g.name}>按 {g.name} 分组</option>)}
                </select>

                <button onClick={() => setShowColToggle(!showColToggle)} className="bg-white border text-gray-600 px-3 py-2 rounded-lg text-sm font-bold shadow-sm">
                  ⚙️ 列显示
                </button>
                {showColToggle && (
                  <div className="absolute top-12 left-0 bg-white border shadow-xl rounded-xl p-3 z-20 w-48 space-y-2">
                    {COL_OPTIONS.map((col) => (
                      <label key={col.id} className="flex items-center space-x-2 text-sm cursor-pointer hover:bg-gray-50 p-1 rounded">
                        <input
                          type="checkbox"
                ...
                <button
                  onClick={() => {
                    setTradeForm({ cashOp: 'add', currency: settings.baseCurrency, amount: '' });
                    setModalType('cash');
                  }}
                ...
                <button
                  onClick={() => {
                    setTradeStep(1);
                    setTradeForm({
                      trackType: 'auto', symbol: '', name: '', quantity: '', price: '',
                      currency: settings.baseCurrency, customPrice: '', customCurrency: settings.baseCurrency, tags: {},
                    });
                    setModalType('buy');
                  }}
            ...
            <div className="bg-white rounded-xl shadow-sm border p-4 flex gap-4 overflow-x-auto">
              {cash.map((c: any) => (
                <div key={c.id} className="min-w-[120px] p-3 bg-gray-50 border rounded-lg">
                  <div className="text-xs text-gray-500 font-bold mb-1">💵 {c.currency}</div>
                  <div className="text-lg font-bold text-gray-800">{formatMoney(c.amount)}</div>
                </div>
              ))}
            </div>
                    ...
                    <tr>
                      {COL_OPTIONS.map((col) =>
                        settings.visibleCols.includes(col.id) && (
                          <th key={col.id} className={`p-4 font-semibold ${['quantity', 'price', 'cost', 'value', 'pnl', 'weight'].includes(col.id) ? 'text-right' : ''}`}>
                            {col.label}
                          </th>
                        )
                      )}
                    ...
                    {Object.entries(groupedHoldings).map(([groupName, groupList]) => (
                      <React.Fragment key={groupName}>
                        ...
                        {groupList.map((h: any) => {
                          ...
                          const pnlPercent = (currentValBase && costValBase > 0) ? (pnlBase! / costValBase) * 100 : null;
                            ...
                            <tr key={h.id} className="hover:bg-gray-50 transition-colors">
                              {settings.visibleCols.includes('symbol') && (
                                <td className="p-4">
                                  <div className="font-bold text-base text-gray-800">
                                    ...
                                    {!isAuto && <span className="text-[10px] bg-yellow-100 text-yellow-700 px-1.5 rounded ml-2 align-middle">自定义</span>}
                                  </div>
                              ...
                              {settings.visibleCols.includes('cost') && (
                                <td className="p-4 text-right text-gray-600">
                                  {h.costPrice.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 4 })}{' '}
                                ...
                                <td className="p-4">
                                  <div className="flex flex-wrap gap-1 max-w-[150px]">
                                    {Object.values(h.tags || {}).filter(Boolean).map((t: any, i) => (
                                      <span key={i} className="text-[10px] bg-gray-100 border text-gray-600 px-2 py-0.5 rounded-full">{t}</span>
                                    ))}
                                  </div>
                                </td>
                              )}
            ...
            <div className="bg-white p-4 rounded-xl shadow-sm border flex flex-wrap items-center gap-3">
              ...
              {[
                ...
                <button
                  ...
                  className={`px-4 py-1.5 rounded-full text-sm font-bold transition-colors border ${allocDims.includes(opt.id) ? 'bg-blue-50 border-blue-200 text-blue-600' : 'bg-white border-gray-200 text-gray-600'}`}
                >
                  {opt.label}
                </button>
              ))}
                  ...
                  <div key={dim} className="bg-white p-6 rounded-xl shadow-sm border h-80 flex flex-col items-center">
                    ...
                    {data.length === 0 ? (
                      ...
                      <ResponsiveContainer width="100%" height="100%">
                        <PieChart>
                          <Pie
                            data={data} cx="50%" cy="50%" innerRadius="40%" outerRadius="80%" paddingAngle={2} dataKey="value"
                            labelLine={false} label={renderCustomizedLabel}
                          >
                            {data.map((_, index) => <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />)}
                          </Pie>
                          <Tooltip formatter={(value: any) => `${formatMoney(value)} ${settings.baseCurrency}`} />
                ...
                {snapshots.length < 2 ? (
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={snapshots}>
                      ...
                      <YAxis domain={['auto', 'auto']} tickFormatter={(v) => formatMoney(v)} tick={{ fontSize: 12, fill: '#6b7280' }} axisLine={false} tickLine={false} width={80} />
                      <Tooltip formatter={(value: any) => `${formatMoney(value)} ${settings.baseCurrency}`} labelStyle={{ color: '#374151', fontWeight: 'bold' }} />
        ...
        {activeTab === 'settings' && (
          <div className="grid md:grid-cols-2 gap-6">
            <div className="bg-white p-6 rounded-xl shadow-sm border">
              ...
              <div className="mb-6">
                <label className="block text-sm font-semibold mb-2 text-gray-700">全局基础货币 (显示单位)</label>
                <select
              ...
              <div className="mb-6">
                <label className="block text-sm font-semibold mb-2 text-gray-700">总额/盈亏 显示精度</label>
                <select
                  className="w-full border rounded-lg p-2 outline-none bg-gray-50 focus:border-blue-500 font-bold"
                ...
                <p className="text-xs text-gray-500 mt-2">
                  仅改变“总资产、总现值、总现金、绝对总盈亏”等汇总数据的显示方式。不影响数量、单价和百分比的精准度。
                </p>
              </div>
            </div>

            <div className="bg-white p-6 rounded-xl shadow-sm border">
              <h3 className="text-lg font-bold mb-6 border-b pb-2">数据安全与迁移</h3>
              ...
              <div className="flex gap-4">
                <button onClick={handleExport} className="flex-1 bg-green-600 text-white py-2 rounded-lg font-bold shadow-sm hover:bg-green-700 transition-colors">
                  ⬇️ 导出备份 (.json)
                </button>
                <input type="file" accept=".json" ref={fileInputRef} onChange={handleImport} className="hidden" />
                <button onClick={() => fileInputRef.current?.click()} className="flex-1 bg-white border border-gray-300 text-gray-700 py-2 rounded-lg font-bold shadow-sm hover:bg-gray-50 transition-colors">
                  ⬆️ 导入备份
                </button>
              </div>
            </div>
            ...
            <div className="bg-white p-6 rounded-xl shadow-sm border md:col-span-2">
              ...
              <div className="flex gap-2 mb-6">
                <input
                  type="text" id="newGroupName" placeholder="新标签组名称 (如: 策略)"
              ...
              <div className="grid md:grid-cols-2 gap-4">
                {tagGroups.map((group: any) => (
                  <div key={group.id} className="p-4 border rounded-lg bg-gray-50 relative">
                    <button onClick={() => setTagGroups(tagGroups.filter((g) => g.id !== group.id))} className="absolute top-2 right-2 text-red-400 text-xs hover:text-red-600 font-bold">删除组</button>
                    ...
                    <div className="flex flex-wrap gap-2 mb-3">
                      {group.values.map((v: string) => (
                        <span key={v} className="bg-white border text-xs px-2 py-1 rounded-full flex items-center gap-1 font-medium">
                          {v}
                          <span className="cursor-pointer text-gray-400 hover:text-red-500 w-4 h-4 flex items-center justify-center rounded-full hover:bg-red-50 transition-colors"
                    ...
                    <input
                      type="text" placeholder="输入新标签名并按回车键..."
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden animate-fade-in">
            <div className="p-5 border-b bg-blue-50 flex justify-between items-center">
              <h3 className="font-bold text-lg text-blue-700 flex items-center">编辑标的 {tradeForm.symbol} <TickerTooltip /></h3>
              <button onClick={() => setModalType(null)} className="text-gray-400 font-bold text-xl hover:text-gray-600">×</button>
            </div>
